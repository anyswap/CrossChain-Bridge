...
func (b *Bridge) StartChainTransactionScanJob() {
	...
	for {
		...
		for h := start + 1; h <= latest; {
			...
			log.Info("====== 888888 ====== Scan chain, get block hash", "", blockHash)
		...
		start = latest
		_ = tools.UpdateLatestScanInfo(b.IsSrc, latest)
		time.Sleep(restIntervalInScanJob)
	}
}

// StartChainTransactionScanJobLegacy scan job
func (b *Bridge) StartChainTransactionScanJobLegacy() {
	log.Info("====== StartChainTransactionScanJob ======")
