// Package conflux implements the bridge interfaces for conflux ecosystem blockchain.
package conflux
...
// Bridge conflux bridge inherit from eth bridge
type Bridge struct {
	*eth.Bridge
}

// NewCrossChainBridge new conflux bridge
...
func (b *Bridge) SetChainAndGateway(chainCfg *tokens.ChainConfig, gatewayCfg *tokens.GatewayConfig) {
	if len(gatewayCfg.FinalizeAPIAddress) == 0 {
		log.Fatal("conflux has no 'FinalizeAPIAddress' gateway to get latest finalized block")
	}
...
func (b *Bridge) VerifyChainID() {
	...
	targetChainID := eth.GetChainIDOfNetwork(eth.ConfluxNetworkAndChainIDMap, networkID)
	isCustom := eth.IsCustomNetwork(networkID)
	if !isCustom && targetChainID == nil {
		log.Fatalf("unsupported conflux network: %v", b.ChainConfig.NetID)
	}
