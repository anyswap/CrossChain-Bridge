// Package arbitrum implements the bridge interfaces for arbitrum ecosystem blockchain.
package arbitrum
...
// Bridge arbitrum bridge inherit from eth bridge
type Bridge struct {
	*eth.Bridge
}

// NewCrossChainBridge new arbitrum bridge
...
func (b *Bridge) VerifyChainID() {
	...
	targetChainID := eth.GetChainIDOfNetwork(eth.ArbitrumNetworkAndChainIDMap, networkID)
	isCustom := eth.IsCustomNetwork(networkID)
	if !isCustom && targetChainID == nil {
		log.Fatalf("unsupported arbitrum network: %v", b.ChainConfig.NetID)
	}
