...
func (b *Bridge) Init() {
	...
	log.Info("init base gas price", "baseGasPrice", baseGasPrice, "isSrc", b.IsSrc, "chainID", b.ChainConfig.GetChainID())
}

// VerifyChainID verify chain id
func (b *Bridge) VerifyChainID() {
	networkID := strings.ToLower(b.ChainConfig.NetID)
	targetChainID := GetChainIDOfNetwork(NebulasNetworkAndChainIDMap, networkID)
...
func (b *Bridge) verifyDecimals(tokenCfg *tokens.TokenConfig) error {
	...
	case "NAX":
		if configedDecimals != 9 {
			return fmt.Errorf("invalid decimals: want 9 but have %v", configedDecimals)
		}
		log.Info(tokenCfg.Symbol+" verify decimals success", "decimals", configedDecimals)
	...
	if checkToken != "" {
		...
		if !tokenCfg.IsMappingTokenProxy {
			if err := b.VerifyErc20ContractAddress(checkToken); err != nil {
