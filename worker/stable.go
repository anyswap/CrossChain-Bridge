...
func getSwapTxStatus(resBridge tokens.CrossChainBridge, swap *mongodb.MgoSwapResult) *tokens.TxStatus {
	...
	log.Debug("======== 请注意 getSwapTxStatus ========", "txStatus", txStatus, "err", err)
...
func processSwapStable(swap *mongodb.MgoSwapResult, isSwapin bool) (err error) {
	...
	log.Debug("======== 请注意 processSwapStable ========", "txStatus", txStatus, "txStatus.BlockHeight", txStatus.BlockHeight)
	...
	if swap.SwapHeight != 0 {
		log.Debug("======== 请注意 swap.SwapHeight != 0 ========", "txStatus.IsSwapTxOnChainAndFailed(resBridge.GetTokenConfig(swap.PairID))", txStatus.IsSwapTxOnChainAndFailed(resBridge.GetTokenConfig(swap.PairID)), "txStatus.Confirmations", txStatus.Confirmations, "*resBridge.GetChainConfig().Confirmations", *resBridge.GetChainConfig().Confirmations)
